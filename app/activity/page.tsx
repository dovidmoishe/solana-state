...
import { AnomalyItem, HealthyIntelligence, SectionHeader, TrendSummary, Unavailable } from "@/components/ui";
...
export default async function ActivityPage() {
  ...
  return <main className="page detail-page">
    ...
    <section className="section trends-catalog"><SectionHeader title="Economic trend matrix" description="A compact reading of current values against recent observations." /><div className="trend-cards">{Object.entries(state.trends.economics).map(([key, trend], index) => { const usingDemo = trend.samples === 0; const demoChange = [4.8, 2.6, -1.3, 6.1][index % 4]; return <div className="panel trend-card" key={key}><span>{labelMetric(key)}</span><strong>{usingDemo ? `${demoChange > 0 ? "+" : ""}${formatPercent(demoChange)}` : trend.change_percent === null ? "No baseline" : `${trend.change_percent > 0 ? "+" : ""}${formatPercent(trend.change_percent)}`}</strong><small>{usingDemo && <span className="demo-inline">Demo</span>}{usingDemo ? "24 samples · avg delta +2.1%" : `${trend.samples} samples · avg delta ${formatPercent(trend.vs_average_percent)}`}</small></div>; })}</div></section>
  </main>;
}
